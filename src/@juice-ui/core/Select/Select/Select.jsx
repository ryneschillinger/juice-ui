...
export default function Select(props) {
  ...
  const renderItem  = (
    ...
    const text = `${item.id ? `${item.id + ". "}` : ""}${item.title}`;
    return (
      <MenuItem
        type={type}
        view={view}
        color={color}
        active={selectedItem && selectedItem.title === item.title}
      ...
      {({ isDark }) => (
        <SelectSource
          ...
          popoverProps={{
            minimal: true,
            popoverClassName: cx(
              type === "dense" ? View["select-dense-popover"] : View["select-popover"],
              filterable && (type === "dense" ? View["select-dense-popover-filterable"] : View["select-popover-filterable"]),
              MenuView[view],
              MenuType[type],
              Color[color]
            )
